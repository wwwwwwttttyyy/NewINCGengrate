...
static Config parse_args(int argc, char** argv) {
    ...
    if (cfg.compute_nm_theta || cfg.theta_coord_opt || cfg.theta_multistart || cfg.theta_phi_scan ||
        ...
        cfg.soft_hard_multistart || cfg.radius_escape_test || !cfg.load_coords.empty()) {
...
static bool run_tests() {
    ...
    cout << "Running v14 tests\n";
    {
        ...
        cfg.compute_nm_theta = true;
        cfg.coord_topology_fallback = "none"; cfg.out = "baseline_hex_v14_test"; cfg.progress_every = 0;
        ...
        ok = ok && pass;
        cout << "Test 1 baseline hex zero: " << (pass ? "PASS" : "FAIL")
    ...
    }
    {
        ...
        cfg.coord_mode = "triangular_lattice"; cfg.topology_from_coords = "voro_radical";
        cfg.compute_nm_theta = true;
        cfg.coord_topology_fallback = "none"; cfg.out = "theta_def_v14_test"; cfg.progress_every = 0;
        RunOutput out = run_baseline_hex(cfg);
        write_outputs(out);
        bool pass = out.valid && out.nm_theta.ran &&
                    out.nm_theta.Theta_face_mean_abs < 1e-10 &&
                    out.nm_theta.Theta_ring_mean_abs < 1e-10 &&
                    out.nm_theta.Theta_face_ring_difference < 1e-10 &&
                    filesystem::exists(filesystem::path(cfg.out) / "theta_definition_compare.txt");
        ok = ok && pass;
        cout << "Test 2 face/ring theta baseline: " << (pass ? "PASS" : "FAIL")
             << " face=" << out.nm_theta.Theta_face_mean_abs
             << " ring=" << out.nm_theta.Theta_ring_mean_abs
             << " diff=" << out.nm_theta.Theta_face_ring_difference << '\n';
    }
    {
        Config cfg;
        ...
        cfg.compute_nm_theta = true;
        cfg.coord_topology_fallback = "none"; cfg.out = "soft_hard_v14_poly_test"; cfg.progress_every = 0;
        RunOutput out = run_coordinate_pipeline(cfg);
        write_outputs(out);
        bool pass = out.valid && out.nm_theta.ran &&
                    isfinite(out.nm_theta.Theta_mean_abs) &&
                    isfinite(out.angle.B_rms) &&
                    isfinite(out.linear.du_rms);
        ok = ok && pass;
        cout << "Test 3 soft polydisperse diagnostics: " << (pass ? "PASS" : "FAIL")
             << " Theta=" << out.nm_theta.Theta_mean_abs
    ...
    }
    {
        ...
        cfg.controlled_perturb_suite = true;
        cfg.perturb_baseline = "hex_equal";
        cfg.perturb_type = "coord_noise";
        cfg.perturb_amp_list = {0.0, 0.01, 0.05};
        cfg.controlled_perturb_out = "controlled_perturb_v14_test.csv";
        cfg.out = "controlled_perturb_v14_test";
        vector<RunOutput> rows = run_controlled_perturb_suite(cfg);
        bool file_exists = filesystem::exists("controlled_perturb_v14_test.csv");
        bool pass = rows.size() == 3 && file_exists &&
                    rows[0].nm_theta.Theta_mean_abs <= rows[1].nm_theta.Theta_mean_abs + 1e-12 &&
                    rows[1].nm_theta.Theta_mean_abs <= rows[2].nm_theta.Theta_mean_abs + 1e-12 &&
                    rows[0].angle.B_rms <= rows[2].angle.B_rms + 1e-12;
        ok = ok && pass;
        cout << "Test 4 controlled perturb suite: " << (pass ? "PASS" : "FAIL")
             << " Theta0=" << rows[0].nm_theta.Theta_mean_abs
             << " Theta2=" << rows[2].nm_theta.Theta_mean_abs
    ...
    }
    {
        ...
        cfg.compute_nm_theta = true; cfg.radius_escape_test = true;
        cfg.escape_max_iter = 1000; cfg.progress_every = 100;
        cfg.out = "radius_escape_v14_test";
        RunOutput out = run_radius_escape_suite(cfg);
        bool file_exists = filesystem::exists(filesystem::path(cfg.out) / "radius_escape_stats.csv");
        bool pass = out.valid && file_exists;
        ok = ok && pass;
        cout << "Test 5 radius escape small: " << (pass ? "PASS" : "FAIL")
             << " ratio=" << out.ricci.radius_ratio
             << " file=" << (file_exists ? 1 : 0) << '\n';
    }
