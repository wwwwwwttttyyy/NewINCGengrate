...
static Config parse_args(int argc, char** argv) {
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        if (arg == "--nx") cfg.nx = stoi(val(arg));
            ...
            cout << "Usage: ./inc_ricci_v7 [--nx N] [--ny N] [--flips N] [--seed N] [--surgery_metric linear]\n";
            exit(0);
        } else {
    ...
    if (cfg.method != "ricci") throw runtime_error("v7 production path supports --method ricci");
...
static void write_outputs(const RunOutput& out) {
    ...
    }
    {
        ...
        f << scientific << setprecision(17);
        f << "INC-Ricci v7 linear-response topology-prediction run\n";
        f << "N: " << mesh.N << "\n";
        f << "E: " << mesh.edges.size() << "\n";
        f << "F: " << mesh.faces.size() << "\n";
        ...
        f << "note: v7 tests linear-response topology prediction and search; it is not a molecular SMC/FIRE or full packing dynamics solver.\n";
    }
    if (out.linear.ran) {
...
static RunOutput refine_after_linear_search(const RunOutput& lin_out, Config cfg) {
    ...
    try {
        ...
        for (int i = 0; i < static_cast<int>(u.size()); ++i) u[i] += lin_out.linear.du[i];
        normalize_u(u);
...
static RunOutput run_linear_surgery(Config cfg) {
    ...
    double current_score = current.linear.S_pred;
    ...
    for (int step = 1; step <= cfg.linear_surgery_steps; ++step) {
        ...
        for (int trial = 0; trial < cfg.linear_surgery_trials_per_step; ++trial) {
            ...
            double S = proposal.linear.S_pred;
            if (S < best_score) {
                best_score = S;
...
static bool run_tests() {
    ...
    cout << "Running v7 tests\n";
    {
        ...
        cfg.surgery_metric = "linear"; cfg.progress_every = 0;
        ...
        ok = ok && pass;
        cout << "Test 1 regular equal linear response: " << (pass ? "PASS" : "FAIL")
    ...
    {
        Config cfg;
        cfg.nx = 8; cfg.ny = 8; cfg.flips = 0; cfg.seed = 2; cfg.radius_seed = 2;
        cfg.radius_mode = "fixed"; cfg.target_dist = "lognormal"; cfg.target_delta = 0.05;
        cfg.target_radius_ratio = 5.0; cfg.surgery_metric = "linear"; cfg.progress_every = 0;
        RunOutput out = execute_run(cfg, false, false);
        bool pass = out.valid && out.linear.ran &&
                    out.linear.rms_K0 > 0.0 &&
                    out.linear.rms_K_lin < out.linear.rms_K0 &&
                    out.linear.du_max < 0.15;
        ok = ok && pass;
        cout << "Test 2 regular weak lognormal predictor: " << (pass ? "PASS" : "FAIL")
             << " rmsK0=" << out.linear.rms_K0
             << " rmsKlin=" << out.linear.rms_K_lin
             << " du_max=" << out.linear.du_max << '\n';
    }
    {
        Config cfg;
    ...
    }
    {
        ...
        cfg.nx = 8; cfg.ny = 8; cfg.flips = 50; cfg.seed = 1; cfg.radius_seed = 101;
        cfg.radius_mode = "fixed"; cfg.target_dist = "lognormal"; cfg.target_delta = 0.25;
        cfg.target_radius_ratio = 5.0; cfg.surgery_metric = "linear"; cfg.proposal_mode = "mismatch";
        cfg.linear_surgery_steps = 20; cfg.linear_surgery_trials_per_step = 10;
        cfg.out = "linear_surgery_v7_test"; cfg.progress_every = 0;
        Config base = cfg; base.proposal_mode = "random";
        RunOutput initial = execute_run(base, false, false);
        double score_initial = initial.linear.S_pred;
        RunOutput final = run_linear_surgery(cfg);
        double score_final = final.linear.S_pred;
        bool pass = final.valid && score_final <= score_initial + 1e-12;
        ok = ok && pass;
        cout << "Test 4 linear surgery small: " << (pass ? "PASS" : "FAIL")
             << " score_initial=" << score_initial
             << " score_final=" << score_final
             << " rmsKlin=" << final.linear.rms_K_lin << '\n';
    }
    {
        Config cfg;
        cfg.nx = 8; cfg.ny = 8; cfg.flips = 50; cfg.seed = 1; cfg.radius_seed = 101;
        ...
        cfg.target_radius_ratio = 5.0; cfg.surgery_metric = "linear"; cfg.proposal_mode = "mismatch";
        cfg.linear_surgery_steps = 20; cfg.linear_surgery_trials_per_step = 10;
        cfg.refine_after_linear = true; cfg.refine_du_bound = 0.15;
        cfg.develop = true; cfg.check_crossings = false; cfg.out = "linear_refine_v7_test";
        RunOutput out = run_linear_surgery(cfg);
        bool pass = out.valid;
        ok = ok && pass;
        cout << "Test 5 linear refinement small: " << (pass ? "PASS" : "FAIL")
             << " finalK=" << out.ricci.state.max_abs_K
             << " physical_v7=" << (out.packing.physical_INC_like_candidate_v6 ? 1 : 0) << '\n';
    }
