...
static Config parse_args(int argc, char** argv) {
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        if (arg == "--nx") cfg.nx = stoi(val(arg));
            ...
            cout << "Usage: ./inc_ricci_v6 [--nx N] [--ny N] [--flips N] [--seed N] [--radius_mode free|fixed|weak]\n";
            exit(0);
        } else {
    ...
    if (cfg.method != "ricci") throw runtime_error("v6 production path supports --method ricci");
    ...
    if (cfg.surgery_metric != "overlap" && cfg.surgery_metric != "radius" && cfg.surgery_metric != "mixed" &&
        cfg.surgery_metric != "v5" && cfg.surgery_metric != "v6") {
        throw runtime_error("surgery_metric must be overlap, radius, mixed, v5, or v6");
    }
    if (cfg.surgery_batch_flips <= 0) throw runtime_error("surgery_batch_flips must be > 0");
    if (cfg.proposal_mode != "random" && cfg.proposal_mode != "highK" && cfg.proposal_mode != "overlap") {
        throw runtime_error("proposal_mode must be random, highK, or overlap");
    }
...
static void write_outputs(const RunOutput& out) {
    ...
    }
    {
        ...
        f << scientific << setprecision(17);
        f << "INC-Ricci v6 distribution-preserving topology-selection run\n";
        f << "N: " << mesh.N << "\n";
        f << "E: " << mesh.edges.size() << "\n";
        f << "F: " << mesh.faces.size() << "\n";
        ...
        f << "note: v6 tests distribution-preserving topology selection; it is not a molecular SMC/FIRE or full packing dynamics solver.\n";
    }
}
...
static bool run_tests() {
    ...
    cout << "Running v6 tests\n";
    {
        ...
        bool pass = out.valid && out.ricci.state.max_abs_K < 1e-12 &&
                    out.packing.intrinsic_endpoint &&
                    out.packing.developed_consistent &&
                    out.packing.nonedge_overlap_free &&
                    out.packing.topology_geometry_valid &&
                    out.packing.strong_INC_candidate &&
                    out.packing.physical_INC_like_candidate_v6 &&
                    !out.packing.noncrystalline_topology_heuristic;
        ok = ok && pass;
        cout << "Test 1 fixed equal regular: " << (pass ? "PASS" : "FAIL")
             ...
             << " physical_v6=" << (out.packing.physical_INC_like_candidate_v6 ? 1 : 0) << '\n';
    }
    {
        Config cfg;
        ...
        cfg.target_radius_ratio = 5.0; cfg.max_iter = 1;
        cfg.develop = true; cfg.progress_every = 0; cfg.check_crossings = false;
        RunOutput out = execute_run(cfg, false, false);
        bool pass = out.valid && out.develop.ran && out.dist.ran && isfinite(out.ricci.state.rms_K);
        ok = ok && pass;
        cout << "Test 2 fixed lognormal random topology: " << (pass ? "PASS" : "FAIL")
             << " maxK=" << out.ricci.state.max_abs_K
             << " rmsK=" << out.ricci.state.rms_K
             << " fixed_good=" << (out.packing.fixed_intrinsic_good ? 1 : 0) << '\n';
    }
    {
        Config cfg;
        cfg.nx = 8; cfg.ny = 8; cfg.flips = 50; cfg.seed = 1; cfg.radius_seed = 101;
        cfg.radius_mode = "weak"; cfg.target_dist = "lognormal"; cfg.target_delta = 0.25;
        cfg.target_radius_ratio = 5.0; cfg.max_iter = 5000; cfg.tol = 1e-8;
        cfg.bounded = true; cfg.u_min = -1.0; cfg.u_max = 1.0;
        cfg.develop = true; cfg.progress_every = 0; cfg.check_crossings = false;
        RunOutput out = execute_run(cfg, false, false);
        bool pass = out.valid && out.dist.ran && isfinite(out.dist.D_sort) && isfinite(out.dist.D_quantile);
        ok = ok && pass;
        cout << "Test 3 weak distribution relaxation: " << (pass ? "PASS" : "FAIL")
             << " finalK=" << out.ricci.state.max_abs_K
             << " D_sort=" << out.dist.D_sort
             << " D_quantile=" << out.dist.D_quantile
             << " preserved=" << (out.packing.weak_distribution_preserved ? 1 : 0) << '\n';
    }
    {
        Config cfg;
        cfg.nx = 8; cfg.ny = 8; cfg.flips = 50; cfg.seed = 1; cfg.radius_seed = 101;
        ...
        cfg.target_radius_ratio = 5.0; cfg.max_iter = 1;
        cfg.develop = true; cfg.progress_every = 0; cfg.check_crossings = false;
        cfg.target_max_ratio = 5.0; cfg.surgery = true; cfg.surgery_steps = 50; cfg.surgery_trials_per_step = 10;
        cfg.surgery_metric = "v6"; cfg.proposal_mode = "highK"; cfg.out = "surgery_v6_fixed_test";
        RunOutput initial = execute_run(cfg, false, false);
        double score_initial = surgery_score(initial, cfg);
        RunOutput final = run_surgery(cfg);
        double score_final = surgery_score(final, cfg);
        bool pass = final.valid && score_final <= score_initial + 1e-12;
        ok = ok && pass;
        cout << "Test 4 fixed surgery score: " << (pass ? "PASS" : "FAIL")
             << " score_initial=" << score_initial
             << " score_final=" << score_final
             << " rmsK=" << final.ricci.state.rms_K << '\n';
    }
    {
        Config cfg;
        cfg.nx = 8; cfg.ny = 8; cfg.flips = 50; cfg.seed = 1; cfg.radius_seed = 1000;
        cfg.develop = true; cfg.progress_every = 0; cfg.check_crossings = false;
        cfg.surgery = true; cfg.surgery_steps = 10; cfg.surgery_trials_per_step = 5;
        cfg.surgery_metric = "v6"; cfg.proposal_mode = "highK";
        cfg.pareto_delta_list = {0.25, 0.35};
        cfg.pareto_ratio_list = {5.0, 8.0};
        cfg.pareto_modes = {"fixed", "weak"};
        cfg.pareto_seeds = {1};
        cfg.pareto_out = "pareto_v6_test.csv";
        cfg.out = "pareto_v6_test";
        vector<RunOutput> rows = run_pareto_scan(cfg);
        bool file_exists = filesystem::exists(cfg.pareto_out);
        bool pass = rows.size() == 8 && file_exists;
        ok = ok && pass;
        cout << "Test 5 small Pareto: " << (pass ? "PASS" : "FAIL")
             << " rows=" << rows.size()
