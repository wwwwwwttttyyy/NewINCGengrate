...
static Config parse_args(int argc, char** argv) {
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        if (arg == "--nx") cfg.nx = stoi(val(arg));
            ...
            cout << "Usage: ./inc_ricci_v5 [--nx N] [--ny N] [--flips N] [--seed N] [--method ricci]\n";
            exit(0);
        } else {
    ...
    if (cfg.method != "ricci") throw runtime_error("v5 production path supports --method ricci");
    ...
    if (cfg.surgery_metric != "overlap" && cfg.surgery_metric != "radius" && cfg.surgery_metric != "mixed" &&
        cfg.surgery_metric != "v5") {
        throw runtime_error("surgery_metric must be overlap, radius, mixed, or v5");
    }
...
static void write_outputs(const RunOutput& out) {
    ...
    }
    {
        ...
        f << scientific << setprecision(17);
        f << "INC-Ricci v5 bounded-surgery packing-diagnostic run\n";
        f << "N: " << mesh.N << "\n";
        f << "E: " << mesh.edges.size() << "\n";
        f << "F: " << mesh.faces.size() << "\n";
        ...
        f << "note: v5 diagnostics test developed disk-packing geometry and bounded-surgery candidates, but they are not a full coordinate-level packing solver.\n";
    }
}
...
static RunOutput run_surgery(Config cfg) {
    ...
    for (int step = 1; step <= cfg.surgery_steps; ++step) {
        ...
        for (int trial = 0; trial < cfg.surgery_trials_per_step; ++trial) {
            ...
            for (int bf = 0; bf < cfg.surgery_batch_flips; ++bf) {
                if (!attempt_random_flip(proposal_mesh, rng)) {
                    flipped = false;
                    break;
                }
            }
    ...
    current.flips_accepted = initial_flips_accepted + surgery_accepts;
    {
        ...
        f << "recommended_interpretation: require all classification flags before claiming physical INC-like packing.\n";
    }
    current.time.total = now_sec() - t_surgery0;
    return current;
}
...
static vector<RunOutput> run_multistart_surgery(Config cfg) {
    ...
    f << "run_id,seed,flips_requested,flips_accepted,final_score,final_max_abs_K,develop_spread,"
         ...
         "valid_disk_packing_candidate,strong_INC_candidate,physical_INC_like_candidate,time_total_sec\n";
    f << scientific << setprecision(17);
...
static bool run_tests() {
    ...
    cout << "Running v5 tests\n";
    {
        ...
        bool pass = out.valid && out.ricci.state.max_abs_K < 1e-12 &&
                    ...
                    out.packing.physical_INC_like_candidate &&
    ...
    }
    {
        Config cfg;
        cfg.nx = 8; cfg.ny = 8; cfg.flips = 50; cfg.seed = 1; cfg.max_iter = 100000;
        cfg.tol = 1e-9; cfg.bounded = true; cfg.u_min = -1.0; cfg.u_max = 1.0;
    ...
    }
    {
        ...
        cfg.tol = 1e-8; cfg.bounded = true; cfg.u_min = -1.0; cfg.u_max = 1.0;
        cfg.develop = true; cfg.progress_every = 0; cfg.check_crossings = false;
        cfg.target_max_ratio = 5.0; cfg.target_delta = 0.25;
        cfg.surgery = true; cfg.surgery_steps = 50; cfg.surgery_trials_per_step = 10;
        cfg.surgery_metric = "v5"; cfg.surgery_ricci_iter = 10000; cfg.out = "surgery_v5_test";
        ...
        ok = ok && pass;
        cout << "Test 5 small surgery: " << (pass ? "PASS" : "FAIL")
             << " score_initial=" << score_initial
             << " score_final=" << score_final
             << " initial_ratio=" << initial.ricci.radius_ratio
             << " final_ratio=" << final.ricci.radius_ratio << '\n';
    }
    {
        Config cfg;
        cfg.nx = 8; cfg.ny = 8; cfg.flips = 50; cfg.seed = 3; cfg.max_iter = 50000;
        cfg.tol = 1e-8; cfg.bounded = true; cfg.u_min = -1.0; cfg.u_max = 1.0;
        cfg.develop = true; cfg.progress_every = 0; cfg.check_crossings = false;
        cfg.target_max_ratio = 5.0; cfg.target_delta = 0.25;
        cfg.multistart_surgery = true; cfg.multistart_count = 3;
        cfg.surgery_steps = 20; cfg.surgery_trials_per_step = 10; cfg.surgery_ricci_iter = 10000;
        cfg.surgery_metric = "v5"; cfg.multistart_out = "multistart_v5_test.csv"; cfg.out = "multistart_v5_test";
        ...
        ok = ok && pass;
        cout << "Test 6 multistart small: " << (pass ? "PASS" : "FAIL")
             << " rows=" << rows.size()
...
int main(int argc, char** argv) {
    try {
        ...
        if (cfg.multistart_surgery) {
            ...
            int valid_disk = 0, strong = 0, physical = 0;
