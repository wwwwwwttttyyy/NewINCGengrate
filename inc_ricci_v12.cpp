...
static bool run_tests() {
    ...
    cout << "Running v12 tests\n";
    {
        ...
        cfg.coord_topology_fallback = "none"; cfg.out = "voro_v12_tri_test"; cfg.progress_every = 0;
        ...
        ok = ok && pass;
        cout << "Test 1 triangular lattice equal radii: " << (pass ? "PASS" : "FAIL")
    ...
    }
    {
        ...
        cfg.coord_topology_fallback = "none"; cfg.out = "voro_v12_soft_test"; cfg.progress_every = 0;
        RunOutput out = run_coordinate_pipeline(cfg);
        bool pass = out.valid && out.coord.ran && out.coord.U_final <= out.coord.U_initial + 1e-12;
        ok = ok && pass;
        cout << "Test 2 soft relax decreases overlap: " << (pass ? "PASS" : "FAIL")
    ...
    }
    {
        ...
        cfg.coord_topology_fallback = "none"; cfg.out = "coord_eval_v12_test"; cfg.progress_every = 0;
        RunOutput out = run_coordinate_pipeline(cfg);
        write_outputs(out);
        bool pass = out.valid && out.coord.ran && out.voro.ran && out.linear.ran &&
                    filesystem::exists(filesystem::path(cfg.out) / "linear_response_summary.txt");
        ok = ok && pass;
        cout << "Test 3 coordinate evaluation N=64: " << (pass ? "PASS" : "FAIL")
    ...
    }
    {
        ...
        cfg.target_dist = "lognormal"; cfg.target_delta = 0.25; cfg.target_radius_ratio = 5.0;
        cfg.coord_mode = "soft_relax"; cfg.relax_steps = 500; cfg.topology_from_coords = "voro_radical";
        cfg.coord_opt = true; cfg.coord_opt_steps = 20; cfg.coord_opt_trials = 4;
        cfg.coord_full_eval_period = 5; cfg.coord_full_eval_top = 1;
        cfg.coord_topology_fallback = "none"; cfg.out = "coord_opt_v12_test"; cfg.progress_every = 0;
        RunOutput out = run_coordinate_optimization(cfg);
        write_outputs(out);
        bool pass = out.valid && out.coord_opt.ran && filesystem::exists(filesystem::path(cfg.out) / "coord_opt_stats.csv") &&
                    out.coord_opt.final.S_X <= out.coord_opt.initial.S_X * 10.0;
        ok = ok && pass;
        cout << "Test 4 small coordinate optimization: " << (pass ? "PASS" : "FAIL")
             << " S_initial=" << out.coord_opt.initial.S_X
             << " S_final=" << out.coord_opt.final.S_X << '\n';
    }
    {
        Config cfg;
        cfg.nx = 8; cfg.ny = 8; cfg.flips = 20; cfg.seed = 1; cfg.radius_seed = 101;
        cfg.target_dist = "lognormal"; cfg.target_delta = 0.25; cfg.target_radius_ratio = 5.0;
        cfg.coord_opt_strategy_compare = true; cfg.relax_steps = 500;
        cfg.coord_opt_steps = 10; cfg.coord_opt_trials = 3; cfg.coord_full_eval_period = 5; cfg.coord_full_eval_top = 1;
        cfg.out = "coord_opt_strategy_v12_test";
        vector<RunOutput> rows = run_coord_opt_strategy_compare(cfg);
        bool file_exists = filesystem::exists("coord_opt_strategy_compare.csv");
        bool pass = rows.size() == 6 && file_exists;
        ok = ok && pass;
        cout << "Test 5 coordinate optimization strategy compare: " << (pass ? "PASS" : "FAIL")
             << " rows=" << rows.size()
