...
static Config parse_args(int argc, char** argv) {
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        if (arg == "--nx") cfg.nx = stoi(val(arg));
            ...
            cout << "Usage: ./inc_ricci_v9 [--nx N] [--ny N] [--flips N] [--seed N] [--angle_precondition 1]\n";
            exit(0);
        } else {
    ...
    if (cfg.radius_mode == "free" &&
        (cfg.angle_precondition || cfg.degree_precondition || cfg.v9_strategy_compare ||
         cfg.parallel_multistart || cfg.degree_target_mode != "none")) {
        cfg.radius_mode = "fixed";
    }
...
static void write_outputs(const RunOutput& out) {
    ...
    }
    {
        ...
        f << scientific << setprecision(17);
        f << "INC-Ricci v9 angle-budget topology-design run\n";
        f << "N: " << mesh.N << "\n";
        f << "E: " << mesh.edges.size() << "\n";
        f << "F: " << mesh.faces.size() << "\n";
        ...
        f << "note: v9 tests angle-budget and degree-radius topology design with linear-response prediction; it is not a molecular SMC/FIRE or full packing dynamics solver.\n";
    }
    if (ab.ran) {
        {
...
static bool run_tests() {
    ...
    }
    {
        ...
        RunOutput out = execute_run(cfg, false, false);
        bool pass = out.valid && out.angle.ran && isfinite(out.angle.B_rms) && out.degree.ran;
        ok = ok && pass;
        cout << "Test 2 random angle diagnostics: " << (pass ? "PASS" : "FAIL")
             << " B_rms=" << out.angle.B_rms
             << " B_max=" << out.angle.B_max << '\n';
    }
    {
        Config cfg;
        cfg.nx = 8; cfg.ny = 8; cfg.flips = 50; cfg.seed = 1; cfg.radius_seed = 101;
        cfg.radius_mode = "fixed"; cfg.target_dist = "lognormal"; cfg.target_delta = 0.25;
        cfg.target_radius_ratio = 5.0; cfg.surgery_metric = "linear"; cfg.degree_target_mode = "none";
        cfg.wDeg_local = 0.0;
        cfg.progress_every = 0;
        RunOutput initial = execute_run(cfg, false, false);
        cfg.angle_precondition = true; cfg.angle_pre_steps = 50; cfg.angle_pre_trials = 20;
        cfg.precondition_order = "angle_only";
        cfg.out = "angle_pre_v9_test";
        RunOutput final = execute_run(cfg, false, false);
        bool pass = initial.valid && final.valid && final.angle.E_B <= initial.angle.E_B + 1e-10;
        ok = ok && pass;
        cout << "Test 3 angle precondition small: " << (pass ? "PASS" : "FAIL")
             << " E_B_initial=" << initial.angle.E_B
             << " E_B_final=" << final.angle.E_B << '\n';
    }
    {
        Config cfg;
        cfg.nx = 8; cfg.ny = 8; cfg.flips = 50; cfg.seed = 1; cfg.radius_seed = 101;
        ...
        cfg.v9_strategy_compare = true; cfg.degree_pre_steps = 20; cfg.degree_pre_trials = 10;
        cfg.angle_pre_steps = 20; cfg.angle_pre_trials = 10;
        cfg.linear_surgery_steps = 5; cfg.linear_candidate_pool = 20; cfg.linear_full_eval_top = 3;
        cfg.strategy_out = "v9_strategy_compare_test.csv"; cfg.out = "v9_strategy_compare_test";
        cfg.progress_every = 0;
        vector<RunOutput> rows = run_v9_strategy_compare(cfg);
        bool file_exists = filesystem::exists(cfg.strategy_out);
        bool pass = rows.size() == 7 && file_exists;
        ok = ok && pass;
        cout << "Test 4 strategy compare small: " << (pass ? "PASS" : "FAIL")
             << " rows=" << rows.size()
    ...
    }
    {
        ...
        cfg.pipeline = "angle_degree_linear"; cfg.degree_target_mode = "quantile";
        cfg.degree_precondition = true; cfg.degree_pre_steps = 5; cfg.degree_pre_trials = 5;
        cfg.angle_precondition = true; cfg.angle_pre_steps = 5; cfg.angle_pre_trials = 5;
        cfg.linear_surgery = true; cfg.linear_surgery_steps = 2; cfg.linear_candidate_pool = 8; cfg.linear_full_eval_top = 2;
        cfg.proposal_mode = "angle_mismatch"; cfg.parallel_multistart = true; cfg.multistart_count = 4; cfg.threads = 2;
        cfg.multistart_out = "v9_multistart_test.csv"; cfg.out = "v9_multistart_test"; cfg.progress_every = 0;
        vector<RunOutput> rows = run_parallel_multistart_v9(cfg);
        bool file_exists = filesystem::exists(cfg.multistart_out);
        bool pass = rows.size() == 4 && file_exists;
        ok = ok && pass;
        cout << "Test 5 parallel multistart small: " << (pass ? "PASS" : "FAIL")
             << " rows=" << rows.size()
             << " file=" << (file_exists ? 1 : 0) << '\n';
    }
...
int main(int argc, char** argv) {
    try {
        ...
        RunOutput out;
        if (cfg.linear_surgery || (cfg.surgery_metric == "linear" && cfg.proposal_mode != "random")) {
