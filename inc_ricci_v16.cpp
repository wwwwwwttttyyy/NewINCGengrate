...
static Config parse_args(int argc, char** argv) {
    ...
    if (cfg.compute_nm_theta || cfg.theta_coord_opt || cfg.theta_multistart || cfg.theta_phi_scan ||
        ...
        cfg.build_defect_atlas || !cfg.load_coords.empty()) {
...
static bool run_tests() {
    ...
    cout << "Running v16 tests\n";
    {
        ...
        cfg.coord_topology_fallback = "none"; cfg.out = "baseline_hex_v16_test"; cfg.progress_every = 0;
        ...
        bool pass = out.valid && out.voro.ran && out.voro.valid_triangulated_torus &&
                    ...
                    out.angle.B_rms < 1e-10 && out.linear.du_rms < 1e-10 &&
                    out.nm_theta.ran && out.nm_theta.Theta_mean_abs < 1e-10 &&
                    out.decomp.ran && fabs(out.decomp.Theta_sum_component) < 1e-10 &&
                    fabs(out.decomp.Theta_shape_component) < 1e-10 &&
                    classify_defect_plane(out.decomp.Theta_sum_component, out.decomp.Theta_shape_component, cfg) == "low_sum_low_shape";
        ok = ok && pass;
        cout << "Test 1 defect-plane hex baseline: " << (pass ? "PASS" : "FAIL")
             << " B_rms=" << scientific << setprecision(6) << out.angle.B_rms
             << " du_rms=" << out.linear.du_rms
             << " Theta_ring=" << out.nm_theta.Theta_ring_mean_abs
             << " sum=" << out.decomp.Theta_sum_component
    ...
    }
    {
        ...
        cfg.perturb_baseline = "hex_equal"; cfg.perturb_type = "coord_noise";
        cfg.perturb_amp_list = {0.0, 0.05};
        cfg.controlled_perturb_out = "controlled_perturb_coord_v16_test.csv";
        cfg.out = "controlled_perturb_coord_v16_test";
        vector<RunOutput> rows = run_controlled_perturb_suite(cfg);
        bool pass = rows.size() == 2 &&
                    rows[1].decomp.Theta_shape_component > 2.0 * max(rows[1].decomp.Theta_sum_component, 1e-14);
        ok = ok && pass;
        cout << "Test 2 defect-plane coord noise: " << (pass ? "PASS" : "FAIL")
             << " sum=" << rows[1].decomp.Theta_sum_component
             << " shape=" << rows[1].decomp.Theta_shape_component << '\n';
    }
    {
        Config cfg;
        ...
        cfg.perturb_amp_list = {0.0, 0.05};
        cfg.controlled_perturb_out = "controlled_perturb_radius_v16_test.csv";
        cfg.out = "controlled_perturb_radius_v16_test";
        vector<RunOutput> rows = run_controlled_perturb_suite(cfg);
        bool pass = rows.size() == 2 &&
                    rows[1].decomp.Theta_sum_component > 0.0 &&
                    rows[1].angle.B_rms > rows[0].angle.B_rms + 1e-6 &&
                    rows[1].linear.du_rms > rows[0].linear.du_rms + 1e-6;
        ok = ok && pass;
        cout << "Test 3 defect-plane radius noise: " << (pass ? "PASS" : "FAIL")
             << " sum=" << rows[1].decomp.Theta_sum_component
             << " B_rms=" << rows[1].angle.B_rms
             << " du_rms=" << rows[1].linear.du_rms << '\n';
    }
    {
        Config cfg;
        ...
        cfg.coord_mode = "soft_relax"; cfg.relax_steps = 300; cfg.topology_from_coords = "voro_radical";
        cfg.compute_nm_theta = true; cfg.decompose_theta = true; cfg.build_defect_atlas = true;
        cfg.escape_max_iter = 500; cfg.progress_every = 0;
        cfg.defect_plane_out = "defect_plane_atlas_v16_test.csv";
        cfg.out = "defect_plane_v16_test";
        vector<DefectPlaneRecord> rows = build_defect_atlas(cfg);
        bool file_exists = filesystem::exists("defect_plane_atlas_v16_test.csv");
        bool has_hex = false, has_coord = false, has_radius = false, has_voro = false;
        for (const auto& r : rows) {
            if (r.sample_class == "hex_baseline") has_hex = true;
            if (r.sample_class == "hex_coord_noise") has_coord = true;
            if (r.sample_class == "hex_radius_noise") has_radius = true;
            if (r.sample_class == "voro_soft_phi_scan") has_voro = true;
        }
        bool pass = file_exists && has_hex && has_coord && has_radius && has_voro;
        ok = ok && pass;
        cout << "Test 4 small defect atlas: " << (pass ? "PASS" : "FAIL")
             << " rows=" << rows.size()
             << " file=" << (file_exists ? 1 : 0) << '\n';
    }
    {
        bool file_exists = filesystem::exists("plot_defect_plane.py");
        bool pass = file_exists;
        ok = ok && pass;
        cout << "Test 5 plot script generated: " << (pass ? "PASS" : "FAIL")
             << " file=" << (file_exists ? 1 : 0) << '\n';
    }
