...
static CoordConfigResult generate_coordinates(const vector<double>& r, const Config& cfg) {
    ...
    if (cfg.coord_mode == "random") {
        ...
        throw runtime_error("coord_mode must be none, random, poisson_like, or soft_relax");
    }
...
static Config parse_args(int argc, char** argv) {
    ...
    if (cfg.coord_mode != "none" && cfg.coord_mode != "random" &&
    ...
    if (cfg.topology_from_coords != "knn" && cfg.topology_from_coords != "delaunay_bruteforce") {
        throw runtime_error("topology_from_coords must be knn or delaunay_bruteforce");
    }
...
static bool run_tests() {
    ...
    }
    {
        ...
        cfg.coord_mode = "poisson_like"; cfg.topology_from_coords = "knn"; cfg.knn_k = 8;
        cfg.coord_topology_fallback = "abstract_seeded"; cfg.coord_flip_steps = 20; cfg.coord_flip_trials = 10;
        cfg.out = "coord_knn_v10_test"; cfg.progress_every = 0;
        RunOutput out = run_coordinate_pipeline(cfg);
        write_outputs(out);
        bool pass = out.valid && out.coord.ran && out.builder.ran &&
                    filesystem::exists(filesystem::path(cfg.out) / "topology_builder_summary.txt");
        ok = ok && pass;
        cout << "Test 2 poisson KNN topology: " << (pass ? "PASS" : "FAIL")
             << " status=" << out.builder.status
             << " valid=" << (out.builder.valid_triangulated_torus ? 1 : 0) << '\n';
    }
    {
        Config cfg;
        cfg.nx = 8; cfg.ny = 8; cfg.seed = 3; cfg.radius_seed = 101;
        cfg.target_dist = "lognormal"; cfg.target_delta = 0.25; cfg.target_radius_ratio = 5.0;
        cfg.coord_mode = "soft_relax"; cfg.relax_steps = 1000; cfg.topology_from_coords = "knn";
        cfg.coord_topology_fallback = "abstract_seeded"; cfg.coord_flip_steps = 10; cfg.coord_flip_trials = 5;
        cfg.out = "coord_soft_v10_test"; cfg.progress_every = 0;
        RunOutput out = run_coordinate_pipeline(cfg);
    ...
    }
    {
        ...
        cfg.target_dist = "lognormal"; cfg.target_delta = 0.25; cfg.target_radius_ratio = 5.0;
        cfg.coord_mode = "soft_relax"; cfg.relax_steps = 500; cfg.topology_from_coords = "delaunay_bruteforce";
        cfg.delaunay_bruteforce = true; cfg.coord_topology_fallback = "abstract_seeded";
        cfg.coord_flip_steps = 5; cfg.coord_flip_trials = 5; cfg.out = "coord_delaunay_v10_test";
        RunOutput out = run_coordinate_pipeline(cfg);
        bool pass = out.coord.ran && out.builder.ran;
        ok = ok && pass;
        cout << "Test 4 brute-force weighted Delaunay controlled: " << (pass ? "PASS" : "FAIL")
             << " status=" << out.builder.status
             << " valid=" << (out.builder.valid_triangulated_torus ? 1 : 0) << '\n';
    }
    {
        ...
        cfg.target_dist = "lognormal"; cfg.target_delta = 0.25; cfg.target_radius_ratio = 5.0;
        cfg.compare_coord_topologies = true; cfg.relax_steps = 500; cfg.coord_flip_steps = 10; cfg.coord_flip_trials = 5;
        cfg.coord_strategy_out = "coord_strategy_compare_v10_test.csv"; cfg.out = "coord_strategy_compare_v10_test";
        vector<RunOutput> rows = run_coord_strategy_compare(cfg);
        bool file_exists = filesystem::exists(cfg.coord_strategy_out);
        bool pass = rows.size() == 6 && file_exists;
        ok = ok && pass;
        cout << "Test 5 coord strategy compare small: " << (pass ? "PASS" : "FAIL")
             << " rows=" << rows.size()
