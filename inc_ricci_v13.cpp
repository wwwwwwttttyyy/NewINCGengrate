...
static Config parse_args(int argc, char** argv) {
    ...
    if (cfg.compute_nm_theta || cfg.theta_coord_opt || cfg.theta_multistart || cfg.theta_phi_scan || !cfg.load_coords.empty()) {
...
static void write_outputs(const RunOutput& out) {
    ...
    if (out.nm_theta.ran) {
        const NMThetaDiagnostics& nm = out.nm_theta;
        {
            ...
            f << "theta_nm_tol: " << cfg.theta_nm_tol << "\n";
...
static CoordConfigResult load_coordinates_from_csv(const string& path, const vector<double>& r, const Config& cfg) {
    ...
    while (getline(f, line)) {
        ...
        cr.x[v] = x;
        cr.y[v] = y;
        wrap_coord(cr.x[v], cr.L);
        wrap_coord(cr.y[v], cr.L);
        seen[v] = 1;
    }
    for (int i = 0; i < cr.N; ++i) {
    ...
    cr.x_initial = cr.x;
    cr.y_initial = cr.y;
    OverlapEnergy ov = compute_overlap_energy(cr.x, cr.y, r, cr.L);
...
static NMThetaDiagnostics compute_nm_theta(const Mesh& mesh, const CoordConfigResult& cr,
    ...
    double sum_sq = 0.0;
    int count = 0;
    nm.Theta_max_abs = 0.0;
    for (int fi = 0; fi < static_cast<int>(mesh.faces.size()); ++fi) {
        const Face& f = mesh.faces[fi];
        double px[3], py[3];
        for (int q = 0; q < 3; ++q) {
            int v = f.c[q].v;
            px[q] = cr.x[v] + static_cast<double>(f.c[q].sx) * cr.L;
            py[q] = cr.y[v] + static_cast<double>(f.c[q].sy) * cr.L;
        }
        for (int q = 0; q < 3; ++q) {
            int qi = (q + 1) % 3;
            int qj = (q + 2) % 3;
            ...
            nm.theta_vertex[vo] += ae;
            nm.theta_vertex_rms[vo] += err * err;
            nm.theta_vertex_count[vo]++;
            sum_abs += ae;
            sum_sq += err * err;
            nm.Theta_max_abs = max(nm.Theta_max_abs, ae);
            count++;
        }
    }
    if (count > 0) {
        nm.Theta_mean_abs = sum_abs / count;
        nm.Theta_rms = sqrt(sum_sq / count);
    }
    for (int i = 0; i < N; ++i) {
    ...
    nm.nm_theta_low = isfinite(nm.Theta_mean_abs) && nm.Theta_mean_abs < cfg.theta_nm_tol;
    nm.ricci_proxy_good = lr.ran && lr.linear_predicted_good;
...
static void soft_relax_micro_steps(CoordConfigResult& cr, const vector<double>& r,
    ...
    for (int step = 0; step < steps; ++step) {
        OverlapEnergy cur = compute_overlap_energy(cr.x, cr.y, r, cr.L, &fx, &fy);
        ...
        for (int i = 0; i < cr.N; ++i) {
            ...
            wrap_coord(cr.x[i], cr.L);
            wrap_coord(cr.y[i], cr.L);
        }
    }
}
...
static CoordConfigResult propose_coord_move(const CoordConfigResult& base, const RunOutput& cur,
    ...
    if (mode == "single_particle") {
        ...
        cand.x[i] += gauss(rng);
        cand.y[i] += gauss(rng);
        wrap_coord(cand.x[i], cand.L);
        wrap_coord(cand.y[i], cand.L);
        ...
        for (int v : verts) {
            ...
            wrap_coord(cand.x[v], cand.L);
            wrap_coord(cand.y[v], cand.L);
        }
...
static bool run_tests() {
    ...
    cout << "Running v13 tests\n";
    {
        ...
        bool pass = out.valid && out.voro.ran && out.voro.valid_triangulated_torus &&
                    ...
                    out.nm_theta.ran && out.nm_theta.Theta_mean_abs < cfg.theta_nm_tol &&
                    out.nm_theta.nm_like_candidate && out.nm_theta.prl_like_strong_candidate;
        ok = ok && pass;
        cout << "Test 1 triangular lattice equal radii NM theta: " << (pass ? "PASS" : "FAIL")
    ...
    }
    {
        ...
        cfg.compute_nm_theta = true;
        cfg.coord_topology_fallback = "none"; cfg.out = "theta_v13_soft_test"; cfg.progress_every = 0;
        RunOutput out = run_coordinate_pipeline(cfg);
        write_outputs(out);
        bool pass = out.valid && out.coord.ran && out.nm_theta.ran &&
                    filesystem::exists(filesystem::path(cfg.out) / "nm_theta_summary.txt");
        ok = ok && pass;
        cout << "Test 2 soft random NM theta: " << (pass ? "PASS" : "FAIL")
             << " Theta=" << out.nm_theta.Theta_mean_abs
    ...
    }
    {
        ...
        cfg.compute_nm_theta = true;
        cfg.coord_topology_fallback = "none"; cfg.out = "theta_corr_v13_test"; cfg.progress_every = 0;
        RunOutput out = run_coordinate_pipeline(cfg);
        write_outputs(out);
        bool pass = out.valid && out.nm_theta.ran &&
                    filesystem::exists(filesystem::path(cfg.out) / "defect_correlation_summary.txt");
        ok = ok && pass;
        cout << "Test 3 correlation summary: " << (pass ? "PASS" : "FAIL")
             << " corrThetaB=" << out.nm_theta.corr_Theta_absB
             << " corrThetadu=" << out.nm_theta.corr_Theta_absdu << '\n';
    }
    {
        Config cfg;
        ...
        cfg.target_dist = "lognormal"; cfg.target_delta = 0.25; cfg.target_radius_ratio = 5.0;
        cfg.coord_mode = "soft_relax"; cfg.relax_steps = 500; cfg.topology_from_coords = "voro_radical";
        cfg.compute_nm_theta = true; cfg.theta_coord_opt = true; cfg.theta_opt_steps = 20; cfg.theta_opt_trials = 4;
        cfg.coord_full_eval_period = 5; cfg.coord_full_eval_top = 1;
        cfg.coord_topology_fallback = "none"; cfg.out = "theta_opt_v13_test"; cfg.progress_every = 0;
        RunOutput out = run_theta_coordinate_optimization(cfg);
        write_outputs(out);
        bool pass = out.valid && out.theta_opt.ran && filesystem::exists(filesystem::path(cfg.out) / "theta_coord_opt_stats.csv");
        ok = ok && pass;
        cout << "Test 4 small theta coordinate optimization: " << (pass ? "PASS" : "FAIL")
             << " Theta_initial=" << out.theta_opt.initial.Theta_mean_abs
             << " Theta_final=" << out.theta_opt.final.Theta_mean_abs << '\n';
    }
    {
        Config cfg;
        cfg.nx = 8; cfg.ny = 8; cfg.flips = 20; cfg.seed = 1; cfg.radius_seed = 101;
        cfg.target_dist = "lognormal"; cfg.target_delta = 0.25; cfg.target_radius_ratio = 5.0;
        cfg.compute_nm_theta = true; cfg.theta_multistart = true; cfg.multistart_count = 4; cfg.threads = 2;
        cfg.relax_steps = 500; cfg.theta_multistart_out = "theta_multistart_v13_test.csv";
        cfg.out = "theta_multistart_v13_test";
        vector<RunOutput> rows = run_theta_multistart(cfg);
        bool file_exists = filesystem::exists("theta_multistart_v13_test.csv");
        bool pass = rows.size() == 4 && file_exists;
        ok = ok && pass;
        cout << "Test 5 theta multistart small: " << (pass ? "PASS" : "FAIL")
             << " rows=" << rows.size()
