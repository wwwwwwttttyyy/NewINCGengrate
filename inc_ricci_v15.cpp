...
static Config parse_args(int argc, char** argv) {
    ...
    if (cfg.compute_nm_theta || cfg.theta_coord_opt || cfg.theta_multistart || cfg.theta_phi_scan ||
        ...
        cfg.decomposition_phi_scan || cfg.topology_contribution_compare || !cfg.load_coords.empty()) {
    ...
    if (cfg.phi_list.empty()) cfg.phi_list = {0.76, 0.78, 0.80, 0.82, 0.84, 0.86};
...
static bool run_tests() {
    ...
    cout << "Running v15 tests\n";
    {
        ...
        cfg.coord_topology_fallback = "none"; cfg.out = "baseline_hex_v15_test"; cfg.progress_every = 0;
        ...
        ok = ok && pass;
        cout << "Test 1 hex decomposition baseline: " << (pass ? "PASS" : "FAIL")
    ...
    }
    {
        ...
        cfg.controlled_perturb_out = "controlled_perturb_coord_v15_test.csv";
        cfg.out = "controlled_perturb_coord_v15_test";
        ...
        ok = ok && pass;
        cout << "Test 2 coord noise decomposition: " << (pass ? "PASS" : "FAIL")
    ...
    }
    {
        ...
        cfg.perturb_amp_list = {0.0, 0.05};
        cfg.controlled_perturb_out = "controlled_perturb_radius_v15_test.csv";
        cfg.out = "controlled_perturb_radius_v15_test";
        vector<RunOutput> rows = run_controlled_perturb_suite(cfg);
        bool pass = rows.size() == 2 &&
        ...
        ok = ok && pass;
        cout << "Test 3 radius noise decomposition: " << (pass ? "PASS" : "FAIL")
             << " sum=" << rows[1].decomp.Theta_sum_component
    ...
    }
    {
        ...
        cfg.coord_mode = "soft_relax"; cfg.relax_steps = 800; cfg.topology_from_coords = "voro_radical";
        cfg.compute_nm_theta = true; cfg.decompose_theta = true;
        cfg.coord_topology_fallback = "none"; cfg.out = "decomp_v15_poly_test"; cfg.progress_every = 0;
        RunOutput out = run_coordinate_pipeline(cfg);
        write_outputs(out);
        bool file_exists = filesystem::exists(filesystem::path(cfg.out) / "local_defects.csv") &&
                           filesystem::exists(filesystem::path(cfg.out) / "local_regression_summary.txt");
        bool pass = out.valid && out.decomp.ran && file_exists;
        ok = ok && pass;
        cout << "Test 4 local defects/regression: " << (pass ? "PASS" : "FAIL")
             << " R2=" << out.decomp.regression_R2
             << " dominant=" << out.decomp.regression_dominant_predictor
    ...
    }
    {
        ...
        cfg.compute_nm_theta = true; cfg.decompose_theta = true; cfg.topology_contribution_compare = true;
        cfg.topology_contribution_out = "topology_contribution_v15_test.csv";
        cfg.linear_surgery_steps = 20; cfg.post_surgery_steps = 20;
        cfg.out = "topology_contribution_v15_test";
        vector<RunOutput> rows = run_topology_contribution_compare(cfg);
        bool file_exists = filesystem::exists("topology_contribution_v15_test.csv");
        bool pass = rows.size() >= 3 && file_exists;
        ok = ok && pass;
        cout << "Test 5 topology contribution compare: " << (pass ? "PASS" : "FAIL")
             << " rows=" << rows.size()
             << " file=" << (file_exists ? 1 : 0) << '\n';
    }
